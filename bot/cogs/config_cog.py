...
class SettingsModal(ui.Modal):
    ...
    def __init__(self, bot: "DailyMessageBot", guild_id: int):
        super().__init__(title="Configure Daily Messages")
        self.bot = bot
    ...
    async def setup_form_fields(self):
        ...
        self.channel_id_input = ui.TextInput(
            ...
            default=(
                str(current_config.get("channel_id", ""))
                if current_config.get("channel_id")
                else None
            ),
        )
...
class ConfigCog(commands.Cog):
    ...
    async def configure_bot_context_menu(
    ...
    @app_commands.command(
        name="toggledaily",
        description="Enable or disable daily messages for this server.",
    )
    ...
    async def toggle_daily(self, interaction: Interaction, enable: bool):
        ...
        try:
            ...
            await self.bot.config_manager.update_config(
                interaction.guild_id, {"enabled": enable}
            )

            status = "enabled" if enable else "disabled"
            await interaction.response.send_message(
                f"✅ Daily messages have been **{status}**.", ephemeral=True
            )
            ...
            await interaction.response.send_message(
                "❌ An error occurred while updating the settings.", ephemeral=True
            )

    @app_commands.command(
        name="status", description="Show current bot configuration for this server."
    )
    @app_commands.checks.has_permissions(manage_guild=True)
    async def show_status(self, interaction: Interaction):
        ...
        try:
            ...
            channel_mention = (
                f"<#{config['channel_id']}>" if config.get("channel_id") else "Not set"
            )
            ...
            embed.add_field(
                name="Time (UTC)", value=config.get("time", "Not set"), inline=True
            )
            embed.add_field(
                name="Message Preview",
                value=(
                    config.get("message", "Not set")[:100] + "..."
                    if len(config.get("message", "")) > 100
                    else config.get("message", "Not set")
                ),
                inline=False,
            )
            ...
            await interaction.response.send_message(
                "❌ An error occurred while retrieving the status.", ephemeral=True
            )
